...
def experience(request, lang=None):
    context = set_language_context(request, lang)
    context.update({
        'experiences': Experience.objects.all()[:4],
        'now': now(),
...
def projects(request, lang=None):
    context = set_language_context(request, lang)
    context.update({
...
def publications(request, lang=None):
    context = set_language_context(request, lang)
    context.update({
        'publications': Publication.objects.all()[:3],
        'now': now(),
...
def cv(request, lang=None):
    context = set_language_context(request, lang)
    context.update({
        'now': now(),
    })
    # Note: cv_file est déjà dans le contexte via set_language_context
